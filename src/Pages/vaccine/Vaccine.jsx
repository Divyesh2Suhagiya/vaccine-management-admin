...
function Vaccine() {
  ...
  const tooltip = (event) => {
    ...
    let tooltipDiv = element.nextElementSibling;
    tooltipDiv.style.display = 'block'
    tooltipDiv.style.top = offsetTop + 20 + 'px'
    tooltipDiv.style.left = offsetLeft + 'px'
  }
    ...
    const columns = [
        ...
        },
        {
            ...
            selector: row => <>
              <div className='position-relative' >
                <span className='overflow-dotted' onMouseOver={tooltip} onMouseLeave={removeTooltip}>{row.endescription}</span>
                <div className='overflow-text'>{row.endescription}</div>
              </div>
            </> ,
            wrap : true,
            width : "200px"
        ...
        },
        {
            ...
            selector: row => <>
                ...
                <button className='btn btn-danger py-1 me-1' onClick={() => deleteVaccine(row._id)}>DELETE</button>
                <button className='btn btn-info py-1' onClick={() => createDuplicate(row)}>DUPLICATE</button>
            </>,
            width:'300px'
        }
    ];

    const createDuplicate = (row) => {
     

      Swal.fire({
        title: "Are you sure?",
        text: "You won't be able to create duplicate this!",
        icon: "question",
        ...
        if (result.isConfirmed) {
          let obj = {
            ...
            description : row.endescription,
            doseML : row.endoseML,
            doseName : row.endoseName,
            fullName : row.enfullName,
            price : row.enprice,
            route : row.enroute,
            shortName : row.enshortName,
            site : row.ensite,
            symptoms : row.ensymptoms,
            vaccineImage : row.vaccineImage
          }
          dispatch(addVaccineDetail(obj))
        }
      });
    }
      ...
      const editVaccine = (editObj) => {
        ...
        for(let key in editObj){
            ...
            if(key == 'date'){
                ...
                let month = Objdate.getMonth()+1 < 10 ? '0' + (Number(Objdate.getMonth())+1) :  Objdate.getMonth()
